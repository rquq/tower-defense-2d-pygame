...
class Enemy:
    """Thực thể kẻ địch cố gắng di chuyển qua bản đồ để tấn công căn cứ."""

    def __init__(self, waypoints, type_name, hp_mult=1.0, speed_mult=1.0):
        """Thiết lập các chỉ số máu, tốc độ và đường đi cho kẻ địch."""
        self.waypoints = waypoints
        self.target_waypoint_idx = 0
        stats = C.ENEMY_TYPES[type_name]
        self.type_name = type_name
        self.max_health = stats['hp'] * C.BASE_ENEMY_HP * hp_mult
    ...
    def update(self):
        """Tính toán việc di chuyển theo lộ trình và xử lý các hiệu ứng trạng thái đang gánh chịu."""
    ...
    def draw(self, surface):
        """Vẽ hình ảnh kẻ địch và thanh máu hiển thị trên đầu."""
        ...
        if asset:
            ...
            target_h = C.GRID_SIZE
            target_w = int(t_width / t_height * target_h)
            if target_w > C.GRID_SIZE:
                target_w = C.GRID_SIZE
