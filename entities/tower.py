...
class Tower:
    """Tháp phòng thủ chính có khả năng tấn công kẻ địch trong tầm bắn."""

    def __init__(self, x, y, tower_type):
        """Khởi tạo tháp với các chỉ số cơ bản dựa trên loại được chọn."""
        stats = C.TOWER_STATS[tower_type]
        self.x = x
        self.y = y
    ...
    def add_gem(self, item):
        """Gắn ngọc nguyên tố vào tháp để thay đổi đặc tính tấn công."""
        if len(self.gems) < 2:
    ...
    def recalculate(self, adjacency_buffs=None):
        """Cập nhật lại các chỉ số chiến đấu sau khi nhận thêm các hiệu ứng bổ trợ."""
        if adjacency_buffs is None:
            adjacency_buffs = []
        self.range = self.base_range
        self.fire_rate = self.base_fire_rate
        self.damage = self.base_damage
        self.crit_chance = C.TOWER_STATS[self.type].get('crit_chance', 0)
        for item in self.gems:
            if item.item_type == C.ItemType.GEM:
                if item.name == 'OVERCLOCK':
    ...
    def update(self, spatial_manager, vfx_manager):
        """Tìm kiếm mục tiêu và thực hiện hành động bắn đạn."""
        if self.cooldown > 0:
            self.cooldown -= 1
    ...
    def find_target(self, spatial_manager):
        """Xác định mục tiêu tốt nhất trong tầm bắn dựa trên chiến thuật đã chọn."""
    ...
    def fire(self):
        """Thực hiện việc tạo ra viên đạn để tấn công kẻ địch."""
        ...
        new_proj = Projectile(self.x, self.y, self.target, final_dmg, self.bullet_speed, self.color, self.color, self.aoe, fire_mod=f_mod, ice_mod=i_mod)
        if is_crit:
            new_proj.color = (255, 255, 255)
        elif self.fire_mod:
            new_proj.color = (255, 120, 0)
        elif self.ice_mod:
            new_proj.color = (150, 255, 255)
        self.projectiles.append(new_proj)
        sounds = {'Earth Cannon': 'shoot_cannon', 'Wind Ballista': 'shoot_arrow', 'Fire Mortar': 'shoot_fire', 'Storm Spire': 'shoot_lightning', 'Frost Cannon': 'shoot_ice'}
    ...
    def draw_design(cls, surface, cx, cy, type_name, color, angle, scale=1.0, is_shadow=False):
        """Vẽ thiết kế ngoại hình của tháp."""
        R = int(14 * scale)
    ...
    def draw(self, surface):
        """Vẽ toàn bộ tháp và các hiệu ứng xung quanh lên bản đồ."""
