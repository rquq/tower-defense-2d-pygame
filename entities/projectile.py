...
class Projectile:
    """Vật thể đạn được bắn ra từ tháp để tấn công kẻ địch."""

    def __init__(self, x, y, target, damage, speed, color, tip_color, aoe=0, fire_mod=False, ice_mod=False):
        """Khởi tạo viên đạn với mục tiêu và các chỉ số sát thương tương ứng."""
        self.x = x
        self.y = y
        self.target = target
        self.damage = damage
    ...
    def update(self, spatial_manager, vfx_manager):
        """Cập nhật quỹ đạo bay của đạn để bám đuổi mục tiêu."""
    ...
    def hit(self, spatial_manager, vfx_manager):
        """Xử lý va chạm khi đạn chạm vào mục tiêu và gây sát thương."""
        targets = []
        if self.aoe > 0:
