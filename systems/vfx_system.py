...
class Particle:
    ...
    def draw(self, surface):
        ...
        if self.is_rect and (abs(self.vx) > 0.1 or abs(self.vy) > 0.1):
            # Directional rectangle aligned with velocity
...
class FloatingText:
    ...
    def draw(self, surface, font):
        ...
        # Scaling effect for crits (Pop Out)
        scale = 1.0
        if self.is_crit:
        ...
        if self.is_crit: sy -= draw_surf.get_height()//2
...
class VFXManager:
    ...
    def spawn_link_particle(self, start_x, start_y, end_x, end_y, color):
        ...
        # Target is always the pinpoint center of the turret
        dx = end_x - sx
        dy = end_y - sy
        dist = math.hypot(dx, dy)
        if dist > 0:
            speed = random.uniform(1.8, 3.2)
            # EXACT life: life * speed = dist => life = dist / speed
            life = int(dist / speed)
            vx = (dx / dist) * speed
            vy = (dy / dist) * speed
